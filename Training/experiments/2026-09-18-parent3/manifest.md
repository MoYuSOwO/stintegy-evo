...
## 结局：废弃于约 208k 步（2026-09-19）

**因预算目标线的语义缺陷废弃，发现者是用户。** 用户原话："重新开始，这没法用了。"

这一炉的世界 `0fa9841` 把目标线画成一条从发车满电到终点的绝对线。二维随机起点约六成落在线下，
线下每少耗一点电都有奖励，一米最多为推进奖励的 3 倍。心跳里预算项一直约为推进项的一半
（熄火前最后一次读数：budget +0.0318，own_progress +0.0496）。它教 Normal 自动还债，
同一个命令会因为之前发生过什么而产生不同的开法。用户约在 150k 步时发现，熄火于约 208k 步
（最后日志：step 208000，α 0.003，1359 tps）。

修复见设计底稿三之勘误，新世界指纹 `8720958`；重点火为 parent3b（`2026-09-19-parent3b`）。
日志归档为本目录的 `run-parent3.log`；检查点留在 `.worktrees/bake-parent3/Training/python/checkpoints/`，不作选材。
