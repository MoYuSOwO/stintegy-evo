...
public sealed class DirectDriveDuelEnvironment
{
    ...
    public const float DefaultAgentStepSeconds =
        1f / DirectDriveController.DefaultDecisionHz;
    ...
    /// <summary>The ego's identity; the controller is what varies.</summary>
    private static readonly DriverProfile EgoProfile = new(
        "training-ego",
        new DriverAbilities()
    );
    ...
    public DirectDriveController EgoDriver =>
    ...
    )
    {
        ...
        _ego = CreateCar(
            ...
            EgoStartS,
            startSpeed,
            new Driver(EgoProfile, _egoDriver),
            egoConfig,
            egoTires,
            EgoStrategy,
            egoStart
    ...
    private static float EstimateStartSpeed(TrackData track, float s)
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            maximumCurvature = MathF.Max(
                maximumCurvature,
                MathF.Abs(track.Sample(s + i * 8f).Curvature)
            );
        }
    ...
    /// <summary>
    /// The car at its starting station, on the centreline and pointing
    /// along it. world-v2 started cars on the racing line's position and
    /// heading; the racing line left the track model with the boundary
    /// migration, so the start is the road's own centre now.
    /// </summary>
    ...
    private static TrackData BuildSpeedwayTrack() =>
        new TrackBuilder(Vector2.Zero, startWidth: 20f)
            .AddStraight(3000f)
            .AddTurn(180f, 400f)
            .AddStraight(3000f)
            .AddTurn(180f, 400f)
