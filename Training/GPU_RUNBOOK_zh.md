...
1. **瓶颈可能在 CPU，不在显卡。** 训练主机用 `Parallel.For` 把 64 条车道铺满所有核，
   物理步进是纯 CPU 活；显卡只加速 SAC 的网络更新。**选机时 CPU 核数和显卡同样重要**——
   8 个 vCPU 配一张 5090 会让显卡闲着等环境。
...
12 车道 × 600 秒，名义 3/3——57 圈、0 旋转、27 圈干净、最快干净圈 1:41.585。
...
**显卡利用率长期很低、tps 与 Mac 相当** 说明瓶颈在 CPU（见〇.1），换多核机器比换卡有用。
