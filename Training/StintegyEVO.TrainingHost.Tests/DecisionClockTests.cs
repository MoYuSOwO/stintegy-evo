...
public sealed class DecisionClockTests
{
    ...
    /// <summary>
    /// And the ego controller must keep no clock of its own, so that a
    /// future edit cannot quietly hand the clock back: however often the
    /// simulation asks it, and with whatever step, it answers with the
    /// command last committed and decides nothing.
    /// </summary>
    [Fact]
    public void TheEnvironmentOwnsTheClock()
    {
        ...
        environment.Step(new[] { 0.3f, 0.6f }, observation);

        var context = environment.Simulation.CaptureFrameContext(environment.Ego);
        var held = environment.EgoDriver.GetControl(in context, 1f / 120f);
        Assert.Equal(environment.Ego.LastInput, held);
        for (int i = 0; i < 50; i++)
        {
            Assert.Equal(held, environment.EgoDriver.GetControl(in context, 0.5f));
        }
    }
