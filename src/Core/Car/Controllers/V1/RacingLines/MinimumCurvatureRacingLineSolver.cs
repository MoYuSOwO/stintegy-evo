...
using Godot;
using StintegyEVO.Core.Car.Controllers.V1.Numerics;
using StintegyEVO.Core.Track;
using StintegyEVO.Util;

namespace StintegyEVO.Core.Car.Controllers.V1.RacingLines;

public sealed class MinimumCurvatureRacingLineSolver : IRacingLineSolver
{
    ...
    public MinimumCurvatureRacingLineSolver()
        : this(OptimizationStepMeters)
    {
    }

    public MinimumCurvatureRacingLineSolver(float optimizationStepMeters)
        : this(optimizationStepMeters, LinearAlgebraBackends.CreateDefault(), QpSolvers.CreateDefault())
    {
    }

    public MinimumCurvatureRacingLineSolver(ILinearAlgebraBackend linearAlgebra, IQpSolver qpSolver)
        : this(OptimizationStepMeters, linearAlgebra, qpSolver)
    {
    }

    public MinimumCurvatureRacingLineSolver(float optimizationStepMeters, ILinearAlgebraBackend linearAlgebra, IQpSolver qpSolver)
    {
    ...
    public RacingLine Generate(TrackData track)
    {
        ...
        if (track.Length < 8)
            return CenterLineRacingLineSolver.Instance.Generate(track);

        int stride = Mathf.Max(1, Mathf.RoundToInt(_optimizationStepMeters / TrackData.StepLength));
        int count = Mathf.Max(4, track.Length / stride);
        ...
        {
            int trackIndex = (i * stride) % track.Length;
        ...
        RacingLine line = BuildTrackAnchoredLine(track, trackIndices, racelineSpline);
    ...
    private static RacingLine BuildTrackAnchoredLine(TrackData track, int[] trackIndices, SplineData racelineSpline)
    {
        const int IntersectionSearchRadius = 2;
        const int IntersectionSamplesPerSegment = 12;

        Vector2[] positions = new Vector2[track.Length];
        ...
        for (int i = 0; i < track.Length; i++)
        {
            ...
            int segmentEnd = segment == lastSegment ? trackIndices[0] + track.Length : trackIndices[segment + 1];
            int wrappedIndex = i < segmentStart ? i + track.Length : i;
            float segmentProgress = segmentEnd <= segmentStart
                ? 0f
                : Mathf.Clamp((wrappedIndex - segmentStart) / (float)(segmentEnd - segmentStart), 0f, 1f);
        ...
        for (int i = 0; i < track.Length; i++)
        {
            Vector2 tangent = GeomUtil.FivePointStencil(
                positions[WrapIndex(i - 2, track.Length)],
                positions[WrapIndex(i - 1, track.Length)],
                positions[WrapIndex(i + 1, track.Length)],
                positions[WrapIndex(i + 2, track.Length)]
            );
            float heading = GeomUtil.NormalizeAngle(tangent.Angle());
            float curvature = GeomUtil.Curvature(
                positions[WrapIndex(i - 1, track.Length)],
                positions[i],
                positions[WrapIndex(i + 1, track.Length)]
            );
    ...
    )
    {
        ...
        float candidateOffset = (point - trackPoint.Center).Dot(trackPoint.Normal);
    ...
    private static float RefineNormalIntersection(SplineData spline, int segment, TrackPoint trackPoint, float lowT, float highT)
    {
    ...
    private static float NormalLineResidual(SplineData spline, int segment, float t, TrackPoint trackPoint)
    {
        Vector2 diff = spline.Evaluate(segment, t) - trackPoint.Center;
        return diff.Cross(trackPoint.Normal);
    }

    private static float ProjectApproximateOffset(SplineData spline, TrackPoint trackPoint, int segment, float segmentProgress)
    {
        Vector2 sampledPoint = spline.Evaluate(segment, segmentProgress);
        return (sampledPoint - trackPoint.Center).Dot(trackPoint.Normal);
    }
    ...
    private sealed class SplineData
    {
        ...
        public static SplineData CreateClosed(Vector2[] points, bool useDistanceScaling, ILinearAlgebraBackend linearAlgebra)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                lengths[i] = Math.Max(a.DistanceTo(b), 1e-6f);
            }
            lengths[n] = lengths[0];
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                normals[i] = normal.LengthSquared() > 1e-12f ? normal.Normalized() : Vector2.Zero;
            }
        ...
        public float Heading(int index, double t)
        {
            ...
            return GeomUtil.NormalizeAngle((float)Math.Atan2(yd, xd));
        }
        ...
        public double[] CalculateSplineLengths()
        {
            ...
            for (int i = 0; i < Count; i++)
            {
                ...
                for (int s = 1; s <= Samples; s++)
                {
                    ...
                    sum += previous.DistanceTo(current);
                    previous = current;
                }
