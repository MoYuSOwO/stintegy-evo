namespace StintegyEVO.Core.Drivers;

public readonly record struct ReferenceLineDriverTelemetry(
    float FrontAxleS,
    float LateralErrorMeters,
    float HeadingErrorRadians,
    float ReferenceCurvature,
    float PreviewCurvature,
    float DesiredCurvature,
    float CurvatureCorrection,
    float PredictedPathLengthMeters,
    float PredictedPathMaximumCurvature,
    float PredictedTerminalLateralErrorMeters,
    float DynamicPredictionLengthMeters,
    bool JoinsReferenceLine,
    float ReferenceLineJoinCurvatureDelta,
    float PathPredictionMilliseconds,
    float RollingSpeedPlanningMilliseconds,
    TrafficSpeedConstraintKind TrafficConstraintKind,
    string? TrafficOpponentId,
    float TrafficConstraintDistanceMeters,
    float TrafficTargetSpeedMetersPerSecond,
    float TrafficConflictTimeSeconds,
    float TrafficCurrentClearanceMeters,
    float TrafficEvaluationDistanceMeters,
    float TrafficFreeArrivalTimeSeconds,
    float TrafficConstrainedArrivalTimeSeconds,
    float TrafficTimeLossSeconds,
    float ReferencePathTargetSpeed,
    float TargetSpeed,
    float ReferenceAcceleration,
    float LossCompensationAcceleration,
    float GradeCompensationAcceleration,
    float SpeedFeedbackAcceleration,
    float DriveAccelerationLimit,
    float DesiredAcceleration,
    float PaceRating,
    float ConsistencyRating,
    float CarControlRating,
    float TireManagementRating,
    float AdaptabilityRating,
    float SessionForm,
    float LapForm,
    float SegmentForm,
    float PlanningPace,
    float EffectivePace,
    float PaceEfficiency,
    float EffectiveControl,
    float EffectiveTireManagement,
    float TireEnergyEfficiency,
    float EffectiveAdaptability,
    float ActualGrip,
    float EstimatedGrip,
    float EstimatedGripScale,
    float BrakeMarkerErrorMeters,
    float LateralTargetErrorMeters,
    float LocalSpeedErrorFraction,
    float FrontBrakeBiasOffset,
    float ControlSeverity,
    float ControlCurvatureCorrection,
    bool IsRecovering
);
