...
/// <summary>
/// The combined-grip limiter, a stability device on the car (see
/// CarConfig.CombinedGripLimiterStrength), reads a tyre's share of its
/// curve, and the curve turns over.
///
...
/// angle. The last three spins measured on the historical training branch were slow-corner exits with the rear
...
/// </summary>
public sealed class CombinedGripLimiterTests
{
    private const float Grip = 10f;
    private static readonly float Normal =
        new TireConfig().GetAccelerationUsage(TireUsageMode.Normal);

    private static readonly CarConfig Fitted = new();

    private static float Limit(
        float request, float grip, float slipAngle, float peakScale,
        float allowance, float speed
    ) => CarPhysics.CombinedGripLimitedLongitudinal(
        Fitted, request, grip, slipAngle, peakScale, allowance, speed
    );

    /// <summary>
    /// Strength zero is the device not fitted: every request comes back
    /// exactly as it went in, on every rung, either side of the peak, at any
    /// speed, pedals either way.
    /// </summary>
    [Fact]
    public void StrengthZeroReturnsEveryRequestUnchanged()
    {
        CarConfig unfitted = new() { CombinedGripLimiterStrength = 0f };
        float[] requests = [-40f, -Grip, -0.3f, 0f, 0.3f, Grip, 40f];
        float[] multiples = [0f, 0.5f, 0.99f, 1f, 1.8f, 4f];
        float[] speeds = [0f, 6f, 35f];
        foreach (TireUsageMode mode in Enum.GetValues<TireUsageMode>())
        {
            ...
            {
                float slip = TireSlipCurve.PeakSlipAngleRadians * multiple;
                Assert.Equal(
                    request,
                    CarPhysics.CombinedGripLimitedLongitudinal(
                        unfitted, request, Grip, slip, 1f, allowance, speed
                    )
                );
            }
        }
    ...
    /// <summary>
    /// Strength is a blend between the raw request and the fully limited
    /// one, so half strength lands halfway.
    /// </summary>
    [Fact]
    public void StrengthBlendsBetweenTheRequestAndTheLimit()
    {
        CarConfig half = new() { CombinedGripLimiterStrength = 0.5f };
        float slip = TireSlipCurve.PeakSlipAngleRadians * 0.5f;
        float full = Limit(Grip, Grip, slip, 1f, Normal, 35f);

        Assert.Equal(
            (Grip + full) * 0.5f,
            CarPhysics.CombinedGripLimitedLongitudinal(
                half, Grip, Grip, slip, 1f, Normal, 35f
            ),
            4
        );
    }
    ...
    public void PastThePeakTheThrottleStaysShut(float multipleOfPeak)
    {
        ...
        Assert.Equal(
            0f,
            Limit(Grip, Grip, slipAngle, 1f, Normal, speed: 35f)
        );
        Assert.Equal(
            0f,
            Limit(Grip, Grip, -slipAngle, 1f, Normal, speed: 35f)
    ...
    public void AtWalkingPaceAPastPeakReadingDoesNotShutTheThrottle()
    {
        ...
        Assert.Equal(
            expected,
            Limit(Grip, Grip, slipAngle, 1f, Normal, speed: 6f),
            4
        );
    }
    ...
    public void BelowThePeakTheCeilingIsWhatItWas()
    {
        ...
        float ceiling = Limit(Grip, Grip, slipAngle, 1f, Normal, speed: 35f);
    ...
    /// rung the limiter trims and for both axles' curves -- the front's peak
    ...
    )
    {
        ...
        for (int i = 0; i <= samples; i++)
        {
            ...
            float ceiling = Limit(Grip, Grip, slipAngle, peakScale, allowance, speed: 35f);
            Assert.True(
    ...
    )
    {
        ...
        Assert.Equal(
            Grip,
            Limit(Grip, Grip, slipAngle, 1f, attack, speed: 35f)
        );
    }
}
